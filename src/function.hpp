...
void CDS_set(vector<pair<int , string> >&CDS_list,vector<pair<int,string> >&CDStmp,unordered_map<string,string>hash)
{
      ...
      for(int i=0;i<CDStmp.size();i++){      
            ...
            if(CDStmp.size()==1){
                  ...
                  string w2 =hash[Vec[0]].substr(stoi(Vec[2])-3,3);
                  if(Vec[4]=="+"){
                        if(w == "ATG" && (w2 == "TAA" || w2 == "TGA" || w2 == "TAG") && (stoi(Vec[2])-stoi(Vec[1])+1)%3==0 ){
                              CDStmp[i].second+="3";
                                     }
                        else{CDStmp[i].second+="4";}
                  }                  
                  else if(Vec[4]=="-"){     
                        if(w2 == "CAT" && (w == "TTA" || w == "TCA" || w == "CTA") && (stoi(Vec[2])-stoi(Vec[1])+1)%3==0 ){
                              CDStmp[i].second+="3";
                                     }
                        else{CDStmp[i].second+="4";}
                  }
