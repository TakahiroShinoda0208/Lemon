...
python /data/yuasa/script/same_line_remover.py genome.pre.txt > genome.txt
rm genome.pre.txt

#改行を取り除いたfastaファイルの作成
python /data/yuasa/script/snap/rmk_fasta_maker_2018NOV11.py $genome
...
do
	...
	python /data/yuasa/script/snap/fasta_pickup_2018NOV11.py $genome $line > ${line}.fasta
