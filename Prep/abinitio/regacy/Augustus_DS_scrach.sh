...
####事前入力####
spec= #パラメータのディレクトリ($AUGUSTUS_CONFIG_PATH/speciesの下に同じ名前が内容に気をつける)
...
#スタートコドンがなかったり、終始コドンが内容なエラーを取り除く(genericはAugustusのマーカーセット)参考：Scipio
...
#/scratch/以下に一時的なディレクトリを作成
mkdir -p /scratch/Augustus
mkdir /scratch/Augustus/${dict}
...
do
	python /data/yuasa/script/augustus_crafty/tab_separate_third_step.py sPlitfile${y}.tMp > /scratch/Augustus/${dict}/sPlitfile${y}.fasta &
done
wait
...
do
        nice -n $nice_value  augustus --species=${spec} /scratch/Augustus/${dict}/sPlitfile${k}.fasta  > /scratch/Augustus/${dict}/abinitio${k}.gff &
done
...
do
        grep -v "#" /scratch/Augustus/${dict}/abinitio${l}.gff > /scratch/Augustus/${dict}/abinitio${l}_shaped.gff &
done

wait

#予測結果のファイルを統合(IDの付け方はアイソフォームを考慮していない)
cat /scratch/Augustus/${dict}/abinitio*_shaped.gff > abinitio_shaped_cat.gff
...
rm -r each_gff
rm -r each_fasta
#/scratch/以下のの一時的なディレクトリの削除
rm -r /scratch/Augustus/${dict}
