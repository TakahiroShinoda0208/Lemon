
####入力情報####
spec= #パラメータのディレクトリ(以前使ったものは不可)
dict= #作業ディレクトリの名前
genome= #Augustus.shのゲノムと同じ
gff= #学習セット
size=`echo 1000` #学習セットのサイズ
s_number=15 #分割するファイルの数
optimize_num=15 #optimize時のthread数
nice_value=5
################

PWD1=`pwd`

mkdir -p mask
cd mask

mkdir $dict
cd $dict

PWD2=`pwd`
echo $PWD2 > ${PWD1}/PASS_information_for_snap_t${size}.txt

#gff3をgtfに変換
python /data/yuasa/script/gff2gtf_better.py $gff > ./input.gtf
awk '$3=="CDS"' ./input.gtf > ./input2.gtf
#gtfをgbに変換
gff2gbSmallDNA.pl input2.gtf $genome 1000 first.gb

#中間ファイル(gtf)の消去
rm input.gtf input2.gtf

######################
#Hintファイルの作成(第一段階)
cat $gff | gffGetmRNA.pl --genome=${genome} --mrna=mrna.fa
blat -noHead $genome mrna.fa est.psl &
######################

##trainingしたパラメータを入れるディレクトリ作成
new_species.pl --species=$spec
#
##スタートコドンがなかったり、終始コドンが内容なエラーを取り除く(genericはAugustusのマーカーセット)参考：Scipo
##--stopCodonExcludedFromCDS=trueはCDSにストップコドンを含めているか否かの設定がetrainingでできる。train.errファイルを確認しエラーが多く検出されていれば確認し>た方が良い。
etraining --species=generic first.gb 2> train.err
fgrep "gene" train.err | cut -f 2 -d " " > bad.etraining-test.lst
filterGenesOut_mRNAname.pl bad.etraining-test.lst first.gb > second.gb
#
##テスト用のファイル切り出し(テストファイルサイズ100)
randomSplit.pl second.gb 100
#
line_num=`grep "LOCUS" second.gb.train | wc -l`
num=`echo $(($line_num - $size))`
##rm tmp.tmp
##limit=`echo 0`
#
randomSplit.pl second.gb.train $num
##randomSplit.pl second.gb.train $size
#	
##training1
etraining --species=$spec second.gb.train.train
##etraining --species=$spec second.gb.train.test
#	
##テスト
augustus --species=$spec second.gb.test | tee firsttest.out
grep -A 22 Evaluation firsttest.out > test_result.txt
#	
##最適化
optimize_augustus.pl --species=$spec second.gb.train.train --nice=${nice_value}  --cpus=${optimize_num} --kfold=${optimize_num}
##optimize_augustus.pl --species=$spec second.gb.train.test
#
wait	
##training2
etraining --species=$spec second.gb.train.train &
##etraining --species=$spec second.gb.train.test

######################
#Hintファイルの作成(第二段階)
#3rd
cat est.psl | filterPSL.pl --best --minCover=80 > est.f.psl
#4th
blat2hints.pl --nomult --in=est.f.psl --out=hints.est.gff &
######################
wait
	
#ファイルの分割
python /data/yuasa/script/augustus_crafty/pre_assembly_first_step.py $genome $s_number
sort -nrk 1 -t "@" list.tMp > list_sort.tMp
wait
python /data/yuasa/script/augustus_crafty/distribute_second_step.py list_sort.tMp $s_number
#
for y in $(seq 1 $s_number)
do
	python /data/yuasa/script/augustus_crafty/tab_separate_third_step.py sPlitfile${y}.tMp > sPlitfile${y}.fasta &
done
wait

#hintファイル用のgff作成
for g in $(seq 1 $s_number)
do
	grep ">" sPlitfile${g}.fasta > tmplist${g}.Tmp
	awk -F ">" '{print $2}' tmplist${g}.Tmp > list${g}.Tmp
	while read line
	do
		grep $line hints.est.gff >> hints${g}.gff
	done < list${g}.Tmp
done 
wait
cp /data/yuasa/tools/augustus/config/extrinsic/extrinsic.E.cfg extrinsic.${spec}.cfg
#
##分割したファイルで予測
for k in $(seq 1 $s_number)
do
        nice -n $nice_value augustus --species=$spec sPlitfile${k}.fasta --hintsfile=hints${k}.gff --extrinsicCfgFile=extrinsic.${spec}.cfg > abinitio${k}.gff &
done
#
wait
#
##分割して予測したファイルを統合
for l in $(seq 1 $s_number)
do
        grep -v "#" abinitio${l}.gff |awk '$2=="AUGUSTUS"' > abinitio${l}_shaped.gff
done
##
wait
##
cat abinitio*_shaped.gff > abinitio_shaped_cat.gff
python /data/yuasa/script/augustus_crafty/re_name_for_AugustusGFF.py abinitio_shaped_cat.gff > AugustusC_abinitio.gtf
rm abinitio_shaped_cat.gff
#
##散らばったファイルの片付け
mkdir -p each_gff
mv abinitio*.gff each_gff
mkdir -p each_fasta
mv *.fasta each_fasta
mkdir -p trash_from_Augustus_crafty
mv *.tMp trash_from_Augustus_crafty/
mv *.Tmp trash_from_Augustus_crafty/
rm -r trash_from_Augustus_crafty
mkdir -p each_hint
mv hints*.gff each_hint
#
#テスト2
augustus --species=$spec second.gb.test | tee second.out
grep -A 22 Evaluation second.out > test_result2.txt
#作成日：2018/11/06(湯淺)
#最終結果のgff：AugustusC_abinitio.gff3
#Augustus Ver.3.3を使用
#Python2系使用
#パラメーターディレクトリの関係もあるのでAugustusは各自インストールしてください。
#コンセプト：GenomeのマルチFASTAファイルを分割することで最後のgffの出力にかかる時間の削減を行う。(出力も結構時間かかる。)
#ファイルの分割はまず、配列が長い順に並べ替えた後、上から順番に指定した数のファイルに分けていく。
#この時の分け方は、ファイルがA,B,C,Dの4つとすると、A→B→C→D→D→C→B→A→A→B.....というような感じで分けてAとDの差が大きくならないようにしている。

